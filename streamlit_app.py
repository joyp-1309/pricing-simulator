"""
Onnyt Pricing & Cost Simulator — v3 (read-only).
Everything live from Airtable: roster + pricing + forecast headcount + SGA (Actual & Forecast).
SGA breakdown is DISPLAYED (Personnel itemized + 62000 lines); 'Total SGA' row drives the net
(no double-count). Actuals used where a month exists in Actual SGA; forecast otherwise (auto-splice).
Onnyt-tagged staff are excluded from the roster (their cost is inside Personnel here).
Deploy: replace streamlit_app.py in the repo, commit, refresh. Token in Secrets. No writes.
"""
...
for _ in range(18):
    ...
    if _m > 12: _m = 1; _y += 1
# map "Jul 2026" <-> "2026-07"
MON_ABBR = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
def col_to_iso(colname):
    try:
        a, y = colname.strip().split()
...
def client_group(mc):
    ...
    if m.strip() == "onnyt": return "EXCLUDE"
...
def load_roster(_token):
    ...
    for r in tbl.all():
        ...
        mc=g(f,"Member Company","MC"); grp=client_group(mc)
...
def load_forecast_hc(_token):
    ...
    for r in Api(_token).table(SIM_BASE,"Sim: Forecast Headcount").all():
        ...
        rec={"mc":gg("MC"),"month":str(gg("Month") or "")[:7],"grptier":gt,
             "group":gg("Client Group") or client_group(gg("MC")),
...
@st.cache_data(ttl=300)
def load_sga(_token):
    """Returns {iso_month: {row_label: value}} merged from Actual SGA + Forecast SGA.
    Actual months win over forecast for the same month (auto-splice)."""
    api=Api(_token)
    def read(table):
        data={}
        for r in api.table(SIM_BASE, table).all():
            f=r["fields"]; label=str(f.get("Financial Row","")).strip()
            for col,val in f.items():
                iso=col_to_iso(col)
                if iso: data.setdefault(iso,{})[label]=val
        return data
    forecast=read("Forecast SGA")     # base layer
    actual=read("Actual SGA")         # overrides where present
    merged={m:dict(v) for m,v in forecast.items()}
    actual_months=set()
    for m,v in actual.items():
        ...
        if a["lever"]=="Attrition":
            ...
            for _ in range(int(a["value"])):
                roster.append({"eid":f"NEW-{month}","mc":mc,"site":"BAGUIO","grptier":gt,
                    ...
                    "group":client_group(mc),"kind":"forecast","mid_usd":mid_for(gt),"hire_month":a["eff"]})
    return roster
...
def run_month(month,base,hires,exits,pm,adjustments,fx,fees,sga_map,onshore_adj):
    ...
    for e in roster:
        ...
        if g=="WW":
            mk=price_over.get("WW|all"); mult=(1+mk) if mk is not None else (1.09 if month>="2027-01" else 1.00)
            rev=full*mult
        elif g in ("OG","NC"):
...
st.caption("Read-only. All data live from Airtable. Break-even is a reference marker. No writes.")
...
with st.sidebar:
    ...
    onshore_adj=st.number_input("Onshore cost +/- ($/mo)",value=0.0,step=1000.0,
        help="Temporary adjustment on top of the SGA Onshore Payroll baseline.")
...
# ---- SGA breakdown table (displayed) ----
st.subheader(f"SGA breakdown — {mi} ({tag})")
PERSONNEL=["Onshore Payroll","Onnyt PH Corporate Payroll","JA Relocation Cost (Cebu Condo)",
           "OG Shared Staff Bonus","Staffing Continency (130K)"]
OTHER=["62100 - Technology","62200 - Travel & Entertainment","62300 - Office Expense",
       "62600 - Professional Fees","62900 - Other G&A Expenses"]
mrow=sga_map.get(mi,{})
def val(lbl):
    for k in mrow:
        if k.strip()==lbl.strip(): return float(mrow[k] or 0)
    return 0.0
prows=[{"Personnel Expense":lbl,"$":val(lbl)} for lbl in PERSONNEL]
prows.append({"Personnel Expense":"— Total Personnel","$":val("61000 - Personnel Expense")})
orows=[{"Other SGA":lbl,"$":val(lbl)} for lbl in OTHER]
orows.append({"Other SGA":"— Total SGA (all-in)","$":val("Total SGA")})
cA,cB=st.columns(2)
cA.dataframe(pd.DataFrame(prows).style.format({"$":"${:,.0f}"}),hide_index=True)
cB.dataframe(pd.DataFrame(orows).style.format({"$":"${:,.0f}"}),hide_index=True)
st.caption("Onshore Payroll is shown combined (DT/GN split not exposed). 'Total SGA' drives the net.")
...
for s in sal_axis:
    ...
    for w in ww_axis:
        ...
        row.append(run_month(mi,base,hires,exits,pm,adj,fx,fees,sga_map,onshore_adj)["net"])
    grid.append(row)
...
# ---- group table ----
st.subheader("Where it comes from — "+("group (shareable)" if "Shareable" in view else "MC level (internal)"))
if "Shareable" in view:
    ...
    st.caption(f"Bench (own COS line): ${r['bench']:,.0f}   ·   Total SGA: ${r['sga']:,.0f}   ·   Active billable heads: {r['billable']}")

st.divider()
st.caption("SGA live from Airtable (Actual + Forecast), auto-spliced by month; breakdown shown, DT/GN never split. "
