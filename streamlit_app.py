...
def run_month(iso, people, hc_moves, latest_actual, lvl_avg, pm, adjustments, fx, fees,
    ...
    raw_rev=defaultdict(float); raw_cost=defaultdict(float); raw_heads=defaultdict(int)  # keyed by ACTUAL bucket, not display-merged
    for e in roster:
        ...
        grp_rev[g]+=rev
        raw_rev[b]+=rev; raw_cost[b]+=full; raw_heads[b]+=1
    ...
    # Break-even / contribution margin — whole-company, but MARGIN-DRIVEN not date-driven.
    # A bucket only joins the break-even pool once it's actually earning real margin (>0.5%,
    # to filter out exact-cost passthrough). Yellow Jacket/Illium (bucket "PASS") never earn
    # margin by design and are NEVER included, even though they're visually merged with WWMK
    # into one "WW" display group elsewhere in the app — that display merge is cosmetic only
    # and must not leak into this calculation, or permanently-zero-margin heads would dilute
    # contribution-per-FTE forever. WWMK (real Westward) runs at 0% margin through Dec 2026
    # (excluded automatically) then 9% from Jan 2027 (included automatically) — no hardcoded
    # date check needed; it self-adjusts to whatever the actual WW markup is, including under
    # a Price Change / MC Markup what-if. Bench + SGA remain company-wide (every group's
    # overhead still needs to be covered by whichever groups actually carry margin).
    def _real_margin(b):
        rv=raw_rev.get(b,0.0); ct=raw_cost.get(b,0.0)
        return rv>0 and (rv-ct)/rv>0.005
    be_buckets=[b for b in ("OG","WWMK","NC") if _real_margin(b)]
    be_gp=sum(raw_rev[b]-raw_cost[b] for b in be_buckets)
    be_heads=sum(raw_heads[b] for b in be_buckets)
    contrib=be_gp/be_heads if be_heads else 0
    be=(bench+total_sga)/contrib if contrib>0 else float("inf")
    # OG-only and WW-only contribution/FTE kept as a diagnostic split — lets you see which
    # group is driving a change in the combined number if the OG/WW headcount mix shifts.
    og_only_contrib=(raw_rev.get("OG",0)-raw_cost.get("OG",0))/raw_heads["OG"] if raw_heads.get("OG") else 0
    ww_only_contrib=(raw_rev.get("WWMK",0)-raw_cost.get("WWMK",0))/raw_heads["WWMK"] if raw_heads.get("WWMK") else 0
    return {"month":iso,"net":net,"gp":gp,"rev":dict(grp_rev),"cost":dict(grp_cost),"bench":bench,
            "sga":total_sga,"billable":bill,"be_heads":be_heads,"be_buckets":be_buckets,"heads":hc,
            "contrib":contrib,"breakeven":be,"og_only_contrib":og_only_contrib,"ww_only_contrib":ww_only_contrib}
...
with st.expander("✏️ Edit SGA for what-ifs (session only — does not save to Airtable)"):
    ...
    # Delta-based: only what you actually CHANGE moves the totals below. The Personnel subtotal and
    # Total SGA shown here are Airtable's own real numbers for this month, plus your edits' net effect —
    # never recomputed from a sum of the detail rows. This keeps things correct even if Airtable's own
    # subtotal row doesn't perfectly tie to its component rows for a given month (forecast months can
    # drift from their components since they're entered independently).
    pers_delta=sum(edited_map.get(c,0.0)-float(base_row.get(c,0) or 0) for c in pers_detail)
    other_delta=sum(edited_map.get(c,0.0)-float(base_row.get(c,0) or 0) for c in other_cats)
    base_pers_subtotal=float(base_row.get(PERSONNEL_SUBTOTAL_LABEL,0) or 0)
    base_total=float(base_row.get("Total SGA",0) or 0)
    disp_pers_subtotal=base_pers_subtotal+pers_delta
    disp_total=base_total+pers_delta+other_delta
    st.markdown(f"**Subtotal — {PERSONNEL_SUBTOTAL_LABEL}: ${disp_pers_subtotal:,.0f}**  _(base ${base_pers_subtotal:,.0f} + your edits above)_")
    st.markdown(f"**Total SGA: ${disp_total:,.0f}**  ·  base Total SGA: ${base_total:,.0f}")
...
# sga_over      = base Total SGA + the NET DOLLAR CHANGE you made this session, for Scenario only.
#                 (Delta-based, not a raw resum — so visiting a month with no edits always equals
#                 base exactly, even if Airtable's own subtotal row doesn't tie to its components.)
base_sga_over={m:float(sga_map.get(m,{}).get("Total SGA",0) or 0) for m in MONTHS}
sga_over={}
for m in MONTHS:
    if m in st.session_state.sga_cat_edits:
        edits_m=st.session_state.sga_cat_edits[m]
        base_m=sga_map.get(m,{}) or {}
        delta_m=sum(v-float(base_m.get(c,0) or 0) for c,v in edits_m.items())
        sga_over[m]=base_sga_over[m]+delta_m
    else:
...
if math.isfinite(r["breakeven"]):
    gap=r["breakeven"]-r["be_heads"]
    m2.metric("Break-even FTEs vs actual",f"{r['breakeven']:.0f} / {r['be_heads']}",f"{'-' if gap<=0 else '+'}{abs(gap):.0f}",delta_color="inverse")
else: m2.metric("Break-even FTEs","unreachable")
m3.metric("Contribution / FTE",f"${r['contrib']:,.0f}",f"{r['contrib']-b['contrib']:+,.0f}")
be_label={"OG":"OG","WWMK":"WW","NC":"New Clients"}
st.caption(f"Break-even pool this month: **{', '.join(be_label[x] for x in r['be_buckets']) or 'none'}** "
           f"(a group joins once its own margin exceeds 0.5% — Yellow Jacket/Illium passthrough never joins) · "
           f"OG-only contribution/FTE: ${r['og_only_contrib']:,.0f} · WW-only contribution/FTE: ${r['ww_only_contrib']:,.0f}")
