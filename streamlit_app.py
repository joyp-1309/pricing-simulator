...
def load_people(_token):
    ...
    for r in api.table(ROSTER_BASE,HR_TABLE).all():
        ...
        people.append({"eid":gv(f,"Employee ID"),"hire":parse_date(gv(f,"Hire Date")),"exit":None,
            ...
            "total_usd":float(tu) if tu not in (None,"") else None,
            "labour_usd":float(lab) if lab not in (None,"") else None,
    ...
    for r in api.table(ROSTER_BASE,ATTR_TABLE).all():
        ...
        people.append({"eid":rv(F_EID),"hire":parse_date(rv(F_HIRE)),"exit":parse_date(rv(F_EFF)),
            "mc":mc,"grptier":gt,"tier":tier_of(gt),"level":level_of(gt),"site":site,
