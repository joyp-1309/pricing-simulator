...
CANON_SGA_CATS=["Onshore Payroll","Onnyt PH Corporate Payroll","JA Relocation Cost (Cebu Condo)",
                "OG Shared Staff Bonus","Staffing Continency (130K)","61000-Personnel",
                "62100-Technology","62200-Travel&Ent","62300-Office","62600-Professional Fees","62900-Other G&A"]
...
with st.expander("✏️ Edit SGA for what-ifs (session only — does not save to Airtable)"):
    sga_edit_month=st.selectbox("Month to edit",MONTHS,
        index=MONTHS.index("2026-07") if "2026-07" in MONTHS else 0,key="sga_edit_month_sel")
    base_row=sga_map.get(sga_edit_month,{}) or {}
    cats=[c for c in CANON_SGA_CATS if c in base_row] or [k for k in base_row.keys() if k!="Total SGA"]
    ...
    edited_total=float(edited_cats["Editable Amount"].sum())
    base_total=float(base_row.get("Total SGA",0) or 0)
    st.caption(f"Editable total for **{sga_edit_month}**: ${edited_total:,.0f}  ·  base Total SGA: ${base_total:,.0f}")
    st.caption("Only the month selected above is edited. Every other month keeps its base Total SGA until you visit it here.")
# Build the per-month total the engine uses: edited category sum if this session touched that month, else base Total SGA
sga_over={}
for m in MONTHS:
    if m in st.session_state.sga_cat_edits:
        sga_over[m]=sum(st.session_state.sga_cat_edits[m].values())
    else:
        sga_over[m]=float(sga_map.get(m,{}).get("Total SGA",0) or 0)

def build(adjs):
    return [run_month(m,people,hc_moves,latest_actual,lvl_avg,pm,adjs,fx,fees,
                      sga_override=sga_over.get(m,0.0)+onshore_adj,sga_map=sga_map) for m in MONTHS]
baseline=build(price_adj); scenario=build(price_adj+user_adj)
