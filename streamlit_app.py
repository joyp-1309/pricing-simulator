...
def run_month(iso, people, hc_moves, latest_actual, lvl_avg, pm, adjustments, fx, fees,
    ...
    # Break-even / contribution margin: OG-only, per Joy's own methodology. WW and passthrough
    # run at ~$0 GP by design this year (full-cost billed straight through), so including their
    # heads in the FTE base would dilute contribution-per-FTE and badly overstate the break-even
    # target — OG is the only group whose economics actually carry margin, so it's the only
    # group that belongs in this specific ratio. (Bench + SGA stay company-wide — every group's
    # overhead still needs to be covered by OG's contribution.)
    og_heads=sum(1 for e in roster if e["bucket"]=="OG")
    contrib=gp["OG"]/og_heads if og_heads else 0
    ...
    return {"month":iso,"net":net,"gp":gp,"rev":dict(grp_rev),"cost":dict(grp_cost),"bench":bench,
            "sga":total_sga,"billable":bill,"og_heads":og_heads,"heads":hc,"contrib":contrib,"breakeven":be}
...
if math.isfinite(r["breakeven"]):
    gap=r["breakeven"]-r["og_heads"]
    m2.metric("Break-even FTEs vs OG actual",f"{r['breakeven']:.0f} / {r['og_heads']}",f"{'-' if gap<=0 else '+'}{abs(gap):.0f}",delta_color="inverse")
