...
def load_people(_token):
    ...
    for r in api.table(ROSTER_BASE,HR_TABLE).all():
        ...
        people.append({"eid":gv(f,"Employee ID"),"hire":parse_date(gv(f,"Hire Date")),"exit":None,
    ...
    for r in api.table(ROSTER_BASE,ATTR_TABLE).all():
        f=r["fields"]
        def rv(fid):
        ...
        # Drop people who exited before the horizon (pre-2026). They never appear in any
        # simulated month, and these are the legacy rows with unpopulated FX (NaN/Infinity).
        if exit_ is not None and exit_ < datetime.date(2026,1,1): continue
...
# editable SGA override grid (session-only)
with st.expander("✏️ Edit SGA for what-ifs (session only — does not save to Airtable)"):
    sga_tot={m:float(sga_map.get(m,{}).get("Total SGA",0) or 0) for m in MONTHS}
    edit_df=pd.DataFrame({"Month":MONTHS,"Total SGA":[sga_tot[m] for m in MONTHS]})
    edited=st.data_editor(edit_df,hide_index=True,disabled=["Month"],key="sga_edit")
    sga_over={row["Month"]:row["Total SGA"] for _,row in edited.iterrows()}
