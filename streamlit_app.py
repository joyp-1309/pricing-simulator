"""
Onnyt Pricing & Cost Simulator — full app (read-only).
Reads live from Airtable (Roster + Sim bases). Compute core is the M0/M2-validated logic.
Levers move a 24-month projection; break-even is a reference, not a target.

Deploy: replace the test streamlit_app.py in your repo with this file, commit, refresh.
Token stays in Streamlit Secrets as AIRTABLE_TOKEN. Nothing is ever written back.
"""
...
def mid_for(gt):
    if gt in MID: return MID[gt]
    return MID_BY_LEVEL.get(level_of(gt), 900.0)

def sss_contribution(gross):
    if gross <= 0: return 0.0
    msc = round(gross/500.0)*500
    msc = max(3000, min(35000, msc))
    return max(265.0, min(3530.0, 2030.0 + (msc-20000)/500.0*50.0))

def loaded_php(gross, hmo):
    if gross <= 0: return 0.0
    return (gross + gross/12.0 + sss_contribution(gross)
            + min(0.025*gross, 2500.0) + 200.0 + (hmo or 0) + NIGHT_DIFF*gross)

def client_group(mc):
    m = (mc or "").lower()
    if "onnyt floater" in m: return "BENCH"
    if m.strip() == "onnyt": return "SGA"
...
def load_roster(_token):
    ...
    for r in tbl.all():
        ...
        out.append({"eid":g(f,"Employee ID"),"mc":mc,"site":site,"grptier":gt,
                    ...
                    "hmo":float(g(f,"HMO",d=0) or 0),"group":client_group(mc),"kind":"existing"})
    return out
...
def load_pricing(_token):
    ...
    for r in Api(_token).table(SIM_BASE, "Sim: Tier Price").all():
        ...
        out.append({"group":gg("Client Group"),"level":gg("Tier"),
                    "method":(gg("Pricing Method") or "").strip(),
...
def price_map(pricing):
    ...
    for p in pricing:
        ...
        key = (p["group"], lvl)
        if key not in pm or p["eff"] >= pm[key]["eff"]:
            pm[key] = p
    return pm
...
def emp_lc(e, month, fx, adjustments):
    ...
    gross = gross*mult + addphp
    return loaded_php(gross, e["hmo"]) / fx

def run_month(month, base, hires, exits, pm, adjustments, fx, nonpayroll_sga, fees):
    ...
    cd = sum(1 for e in roster if e.get("site") in ("CEBU","DVO") and e["group"] != "SGA")
    hc = sum(1 for e in roster if e["group"] != "SGA")
    per_head_fac = (fac_b + cd*fac_r)/hc if hc else 0.0
    grp_rev = defaultdict(float); grp_cost = defaultdict(float); bench = 0.0; sga_pay = 0.0
    for e in roster:
        lc = emp_lc(e, month, fx, adjustments)
        g = e["group"]; full = lc + seat + per_head_fac
        if g == "SGA": sga_pay += full; continue
        ...
        if g == "WW":
            ...
            rev = price_over.get("AH|all", 1600)
        else: rev = 0.0
        grp_rev[g] += rev
    sga = nonpayroll_sga + sga_pay
...
def project(base, hires, exits, pm, adjustments, fx, sga, fees):
    return [run_month(m, base, hires, exits, pm, adjustments, fx, sga, fees) for m in MONTHS]
...
st.caption("Read-only decision tool. Levers move a 24-month projection; break-even is a reference marker. "
           "Nothing is written back to Airtable.")
...
with st.sidebar:
    ...
    nonpayroll_sga = st.number_input("Non-payroll SGA ($/mo)", value=48000.0, step=1000.0,
        help="Corporate offshore payroll and Bench are computed live from the roster. "
             "This is the remaining SGA (IT, travel, office, G&A).")
    ...
    st.caption("Prefilled from Airtable. Overwrite to test a re-price.")
...
lever = c1.selectbox("Lever", ["Salary %","Salary PHP","Promotion","Attrition","FX Override"])
...
baseline = project(base, hires, exits, pm, price_adjustments, fx, nonpayroll_sga, fees)
scenario = project(base, hires, exits, pm, price_adjustments+user_adj, fx, nonpayroll_sga, fees)
...
if math.isfinite(r["breakeven"]):
    ...
    m2.metric("Break-even FTEs", "unreachable")
...
for s in sal_axis:
    row = []
    for w in ww_axis:
        adj = [a for a in (price_adjustments+user_adj) if a["scope_value"] != "WW|all"]
        adj = adj + [{"lever":"Price Change","scope_type":"","scope_value":"WW|all","value":w,"eff":"2027-01","end":None},
                     {"lever":"Salary %","scope_type":"All","scope_value":"","value":s,"eff":"2027-07","end":None}]
        row.append(run_month(mi, base, hires, exits, pm, adj, fx, nonpayroll_sga, fees)["net"])
    grid.append(row)
...
if "Shareable" in view:
    ...
    st.caption(f"Bench (own COS line): ${r['bench']:,.0f}   ·   SGA: ${r['sga']:,.0f}   ·   "
               f"Active billable heads: {r['billable']}")

st.divider()
st.caption("Cost formulas M0-validated. WW = passthrough thru Dec-2026 then full-COS×1.09. "
           "OG L4 = labour×1.15 + seat + facility. Bench & Corporate Payroll computed live from roster tags. "
           "Read-only — no writes to Airtable.")
